...
const cvsFiltration = (arr) => {
    // write your code here
    let cvArr=arr.filter(cvArr=>{
        return cvArr.yearsOfExperience>4 && cvArr.tech==="JS";
     }).map(cvArr=>{
         return {
             fullName:`${cvArr.firstName} ${cvArr.LastName}`,
             tech:cvArr.tech
         };
     })
     return cvArr;
...
const skillComparison = (arr1, arr2) => {
    // write your code here
    let newArray=arr1.filter(skill=>!arr2.includes(skill))
    .concat(arr2.filter(skill => !arr1.includes(skill)));
    // write your code here
    return newArray;
